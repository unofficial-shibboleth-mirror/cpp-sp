...
extern "C" int shib_check_user(request_rec* r)
{
    ...
    ap_log_rerror(APLOG_MARK,APLOG_DEBUG|APLOG_NOERRNO,0,r,"shib_check_user: session check for %s",targeturl);
    ...
    if (!shib_cookie.first) {
        ap_log_rerror(APLOG_MARK,APLOG_CRIT|APLOG_NOERRNO,0,r,
            "shib_check_user: no cookieName set for %s", application_id.second);
        return HTTP_INTERNAL_SERVER_ERROR;
    }
    ...
    if (cookies) {
        ap_log_rerror(APLOG_MARK,APLOG_DEBUG|APLOG_NOERRNO,0,r,"shib_check_user() cookies found: %s",cookies);
    ...
    ap_log_rerror(APLOG_MARK,APLOG_DEBUG|APLOG_NOERRNO,0,r,"shib_check_user(): session successfully verified");
    ...
    try {
        ...
        ap_log_rerror(APLOG_MARK,APLOG_ERR|APLOG_NOERRNO,0,r,"shib_check_user(): %s", e.what());
        ...
        ap_log_rerror(APLOG_MARK,APLOG_ERR|APLOG_NOERRNO,0,r,"shib_check_user(): caught unexpected error");
        markupProcessor.insert("errorType", "Attribute Processing Error");
        markupProcessor.insert("errorText", "Unexpected Exception");
        markupProcessor.insert("errorDesc", "An error occurred while processing your request.");
        return shib_error_page(r, application, "rm", markupProcessor);
    ...
    if (settings.second) {
        ...
        if (!settings.second->authorized(assertions)) {
            ...
            ap_log_rerror(APLOG_MARK,APLOG_ERR|APLOG_NOERRNO,0,r,"shib_check_user(): access control provider denied access");
    ...
    apr_table_set(r->headers_in,"Shib-Application-ID",application_id.second);
        ...
        while (statements.hasNext()) {
            ...
            while (attrs.hasNext()) {
                ...
                if (!strcmp(wrapper->getHeader(),"REMOTE_USER") && vals.hasNext())
                    r->user=apr_pstrdup(r->pool,vals.next().c_str());
                else {
                    int it=0;
                    char* header = (char*)apr_table_get(r->headers_in, wrapper->getHeader());
                    if (header) {
                        header=apr_pstrdup(r->pool, header);
                        it++;
                    }
                    else
                        header = apr_pstrdup(r->pool, "");
                    for (; vals.hasNext(); it++) {
...
{
    // With 2.x, this handler always runs, though last. We check if shib_check_user ran,
    // because it will detect a SHIRE request and dispatch it directly.
    void* data;
    apr_pool_userdata_get(&data,g_UserDataKey,r->pool);
    if (data==(const void*)42) {
        ap_log_rerror(APLOG_MARK,APLOG_DEBUG|APLOG_NOERRNO,0,r,"shib_post_handler skipped since check_user ran");
        return DECLINED;
    }
...
int shib_handler(request_rec* r, const IApplication* application, const IPropertySet* sessionProps, SHIRE& shire)
{
    ...
    ap_log_rerror(APLOG_MARK,APLOG_DEBUG|APLOG_NOERRNO,0,r,"shib_handler() Beginning SHIRE processing");
    ...
    try {
        ...
        if (!strcasecmp(r->method,"GET")) {
            ...
            apr_table_setn(r->headers_out, "Location", apr_pstrdup(r->pool,areq));
            return HTTP_MOVED_TEMPORARILY;
        }
        ...
        if (!ct || strcasecmp(ct, "application/x-www-form-urlencoded"))
            throw ShibTargetException (SHIBRPC_OK,
				 apr_psprintf(r->pool, "blocked bad content-type to SHIRE POST processor: %s", (ct ? ct : "")));
        ...
        //apr_hard_timeout("[mod_shib] CGI Parser", r);
        ...
        if (status->isError()) {
            ...
            if (status->isRetryable()) {
                ...
                apr_table_setn(r->headers_out,"Location",apr_pstrdup(r->pool,shire.getAuthnRequest(elements.second)));
                return HTTP_MOVED_TEMPORARILY;
            }
        ...
        ap_log_rerror(APLOG_MARK,APLOG_DEBUG|APLOG_NOERRNO,0,r,
            "shib_handler() POST process succeeded.  New cookie: %s", cookie.c_str());
        ...
        apr_table_setn(r->headers_out, "Location", apr_pstrdup(r->pool,elements.second));
        return HTTP_MOVED_TEMPORARILY;
    }
    catch (ShibTargetException &e) {
        ap_log_rerror(APLOG_MARK,APLOG_DEBUG|APLOG_NOERRNO,0,r,"shib_post_handler(): %s", e.what());
        ...
        ap_log_rerror(APLOG_MARK,APLOG_DEBUG|APLOG_NOERRNO,0,r,"shib_handler(): unexpected exception");
        markupProcessor.insert("errorType", "Session Creation Service Error");
    ...
    ap_log_rerror(APLOG_MARK,APLOG_ERR|APLOG_NOERRNO,0,r,"shib_handler() server error");
    return HTTP_INTERNAL_SERVER_ERROR;
}
...
extern "C" int shib_auth_checker(request_rec *r)
{
    ...
    //XXX
    ap_log_rerror(APLOG_MARK,APLOG_DEBUG|APLOG_NOERRNO,0,r,"REQUIRE nelts: %d", reqs_arr->nelts);
    ap_log_rerror(APLOG_MARK,APLOG_DEBUG|APLOG_NOERRNO,0,r,"REQUIRE all: %d", dc->bRequireAll);
				...
				apr_pool_t* ptemp, server_rec* s)
{
    ...
    ap_log_error(APLOG_MARK,APLOG_DEBUG|APLOG_NOERRNO,0,s,"shib_post_config() starting");

    if (g_Config) {
        ap_log_error(APLOG_MARK,APLOG_INFO|APLOG_NOERRNO,0,s,"shib_post_config(): already initialized");
        return OK;
    }
    ...
    try {
        ...
        if (!g_Config->init(g_szSchemaDir,g_szSHIBConfig)) {
            ap_log_error(APLOG_MARK,APLOG_CRIT|APLOG_NOERRNO,0,s,"shib_post_config(): already initialized!");
            exit(1);
        }
